...
class CarLocalDataSource {
  ...
  List<CarModel> getAllCars() {
    return _carsData.values.expand((cars) => cars).toList();
  }

   
  List<CarModel> getCarsByBrand(String brand) {
    return _carsData[brand] ?? [];
  }

   
  CarModel? getCarById(String carId) {
    try {
      return getAllCars().firstWhere((car) => car.id == carId);
    } catch (e) {
      return null;
    }
  }
  ...
  List<String> getAllBrands() {
    return _carsData.keys.toList();
  }

   
  final Map<String, List<CarModel>> _carsData = {
    "Mercedes": [
      CarModel(
        id: "mercedes_1",
        name: "AMG GT",
